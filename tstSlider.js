const {genId} = prxz.func.other
const sldID = genId()
const wGuid = w.general.renderTo
const pathPics = `custom/images/MISO/`
const dataset = getData(w.data.primaryData.items)
...
function renderWidget(id){
    const widget = document.getElementById(wGuid)
    const textContent = genSLider()
    widget.innerHTML = textContent
    initSlider()
}

function getData(raw){
    return raw.map(v => pathPics + v.values[1].replace('\\', '/'))
}
...
function genSLider(){
    ...
    <style>${sliderStyle(sldID)}</style>`
}

function sliderStyle(id){
    const s = `#sld-${id}`
    return `
...
function initSlider() {
    ...
    function nextSlide() {
        ...
        // Проверяем, не дошли ли мы до копии первого слайда в конце
    ...
    function openFS(startRealIndex) {
        ...
        let fsIndex = Math.max(0, Math.min(total - 1, startRealIndex)) + 1 // +1 из-за копии в начале
